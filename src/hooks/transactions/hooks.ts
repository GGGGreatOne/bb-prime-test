...
export function useSortedRecentTransactions() {
	...
	const pendingTransactions = useMemo(() => sortedRecentTransactions.filter(tx => !tx.receipt), [sortedRecentTransactions])
	const confirmedTransactions = useMemo(() => sortedRecentTransactions.filter(tx => tx.receipt), [sortedRecentTransactions])
